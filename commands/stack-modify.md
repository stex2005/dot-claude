...
`docs/stacked-pr-workflow.md#workspace-and-manifest-resolution`. The manifest is read and
...
cross-repo correlation (`docs/stacked-pr-workflow.md#manifest-schema`) from the branches
...
- **If both are empty**, every branch from before is still present under the same name.
  ...
  (`docs/stacked-pr-workflow.md#manifest-schema`), not positional order, so shuffling the
...
- **Otherwise, for each name in `$dropped_or_renamed`, ask the user**: was `<name>`
  ...
  the drop snippet from `docs/stacked-pr-workflow.md#manifest-writes` with `$r` = this
